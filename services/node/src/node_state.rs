...
mod tests {
    ...
    fn save_then_load_roundtrip() {
        ...
        let s = NodeState {
            ...
            node_token: "tok-xyz".into(),
        };
        save_to(&p, &s).unwrap();
        let got = load_from(&p).unwrap().unwrap();
        assert_eq!(got.node_id, "abc-123");
        assert_eq!(got.node_token, "tok-xyz");
    }

    #[test]
    fn empty_fields_treated_as_missing() {
        let dir = tempdir_like();
        let p = dir.join("empty.json");
        fs::write(&p, br#"{"node_id":"","node_token":""}"#).unwrap();
        assert!(load_from(&p).unwrap().is_none());
    }
    ...
    fn save_overwrites_existing() {
        ...
        .unwrap();
        let got = load_from(&p).unwrap().unwrap();
        assert_eq!(got.node_id, "id2");
        assert_eq!(got.node_token, "t2");
    }

    fn tempdir_like() -> PathBuf {
        let base = std::env::temp_dir().join(format!(
            "lingcdn-node-state-test-{}-{}",
            std::process::id(),
