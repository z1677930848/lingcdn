...
impl CompiledWaf {
            ...
            match policy.scope.as_str() {
                "global" => {
                    for rule in &policy.rules {
                        ...
                        let ua_contains_lower = rule
                            .ua_contains
                            .as_ref()
                            .and_then(|s| if s.is_empty() { None } else { Some(s.to_ascii_lowercase()) });
                ...
                "domain" => {
                    let Some(ref id) = policy.scope_id else { continue };
    ...
    pub fn is_ip_whitelisted(&self, client_ip: Option<&str>, client_addr: Option<IpAddr>) -> bool {
        ...
        let Some(addr) = client_addr else { return false };
...
impl ConfigHolder {
    ...
    pub fn update(&self, config: RuntimeConfig) {
        ...
        *self.state.write() = Some(Arc::new(RuntimeState { config, router, waf }));
    }
...
pub fn load_node_config() -> Result<NodeConfig> {
	...
	if env::var("LISTEN_ADDR").is_err() && config.tls_enabled && config.listen_addr.trim() == "0.0.0.0:80" {
...
fn apply_env_overrides(config: &mut NodeConfig) {
	...
	if let Ok(v) = env::var("CONTROL_TLS_ENABLED") {
		config.control_tls_enabled = matches!(
	...
	if let Ok(v) = env::var("DISK_CACHE_RECREATE_ON_EXCEED") {
		config.disk_cache_recreate_on_exceed = matches!(
			v.to_ascii_lowercase().as_str(),
			"1" | "true" | "yes" | "on"
		);
	}
	...
	if let Ok(v) = env::var("XDP_RATE_LIMIT_ENABLED") {
		config.xdp_rate_limit_enabled = matches!(
			v.to_ascii_lowercase().as_str(),
			"1" | "true" | "yes" | "on"
		);
	}
	if let Ok(v) = env::var("XDP_SYN_FLOOD_ENABLED") {
		config.xdp_syn_flood_enabled = matches!(
			v.to_ascii_lowercase().as_str(),
			"1" | "true" | "yes" | "on"
		);
	}
