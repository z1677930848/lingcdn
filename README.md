...
本项目采用 [Apache License 2.0](LICENSE) 开源许可证发布。你可以：

- 自由使用、修改、分发源码或编译产物（含商业用途）
- 在衍生作品中 **保留版权声明、许可证副本和 [NOTICE](NOTICE) 的归属声明**
- 对修改的文件加注明显的 "changed by ..." 标记

本许可证同时授予了你对本项目相关专利的使用权，但**不授予对项目名称 "LingCDN" 的商标使用权**。若计划以 "LingCDN" 名称对外提供商业服务，请另行与作者协商。

完整条款以仓库根的 [`LICENSE`](LICENSE) 为准。
